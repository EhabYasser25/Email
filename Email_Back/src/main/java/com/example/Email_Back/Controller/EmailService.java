...
public class EmailService {
    ...
    public ResponseEntity<EmailHeader[]> filter(@RequestParam(value = "userEmail") String userEmail,
                                                   ...
                                                   @RequestParam(value = "compared") int compared) {
        String[] userEmailsIDS = {};
        if(!userCache.exists(userEmail))
            return ResponseEntity.status(HttpStatus.NOT_FOUND).body(null);
        User user = this.userCache.retrieve(userEmail);
        ...
        switch(filterType) {
            case "hasAttachments": criteria = new CriteriaHasAttachments(bool);
            case "read": criteria = new CriteriaRead(bool);
            case "date": criteria = new CriteriaDate(compared, bool);
            case "bodyLength": criteria = new CriteriaBodyLength(compared, bool);
        }
